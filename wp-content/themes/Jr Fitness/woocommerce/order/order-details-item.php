...
<?php echo wp_kses_post(apply_filters('woocommerce_order_item_name', $product_permalink ? sprintf('<a class="title" href="%s">%s</a>', $product_permalink, $item->get_name()) : $item->get_name(), $item, $is_visible));
...
<?php if ($product->get_sku()) : ?><div class="sku">SKU: <?php echo $product->get_sku(); ?></div><?php endif; ?>
						<?php
						$data_berocket_brand = get_the_term_list($product->get_id(), 'berocket_brand', '', ', ');
						if ($data_berocket_brand) : ?><div class="brand">Brand: <?php echo get_the_term_list($product->get_id(), 'berocket_brand', '', ', '); ?></div><?php endif; ?>
