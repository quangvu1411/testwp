...
jQuery(document).on('ready', function(){
    ...
    $overlay = jQuery('.ffxf-modal-overlay');
    ...
    /* Modal generate SKU by category */
    $modal_generate_category = jQuery('.modal_generate_category');
    ...
    jQuery('.close_category').on('click', function(){
        $overlay.removeClass('state-show');
        $modal_generate_category.removeClass('state-appear').addClass('state-leave');
    });

    jQuery('.open_category').on('click', function(){
        $overlay.addClass('state-show');
        $modal_generate_category.removeClass('state-leave').addClass('state-appear');
    });

    var select_cat_total = jQuery('.modal_generate_category .ps').text();
    function ifselect(){
        var num_category_total = parseInt(select_cat_total.replace(/\D+/g,""));
        if( jQuery('#product_cat > option:selected').val() === ''){
            jQuery('.generate_button_category').attr('disabled','disabled').addClass('disabled');
            jQuery('.modal_generate_category .ps').html(ffxf_settings_locale.ffxf_text_category_4 + ' ' + num_category_total + ' ' + ffxf_settings_locale.ffxf_text_category_5 + ' ' + ffxf_settings_locale.ffxf_text_category_6);
        }else{
            select_cat = jQuery('#product_cat option:selected').text();
            jQuery('.generate_button_category').removeAttr('disabled','disabled').removeClass('disabled');
            var num_category = parseInt(select_cat.replace(/\D+/g,""));
            var text_category = select_cat.replace(/[^A-z]/g, '');
            jQuery('.modal_generate_category .ps').html(ffxf_settings_locale.ffxf_text_category_1 + ' <b>' + text_category + '</b> ' + ffxf_settings_locale.ffxf_text_category_2 + ' <b><span id="num_category">' + num_category + '</span></b> ' + ffxf_settings_locale.ffxf_text_category_3);
        }
    }
    ifselect();
